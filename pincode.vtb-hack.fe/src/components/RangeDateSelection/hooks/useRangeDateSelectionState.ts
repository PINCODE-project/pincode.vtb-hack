...
import {
	SECOND,
	YEAR,
} from "../../../../../../../../../../../Downloads/date-components-main/src/components/utils/constants.ts";
import {
	constrainValue,
	createPlaceholderValue,
} from "../../../../../../../../../../../Downloads/date-components-main/src/components/utils/dates.ts";
import { useDefaultTimeZone } from "../../../../../../../../../../../Downloads/date-components-main/src/components/utils/useDefaultTimeZone.ts";
import { alignDateTime } from "../utils/date.ts";

function getViewportIntervalFromDates(start: DateTime, end: DateTime, numberOfIntervals = 4, placeInViewport = 0.5) {
...
export function useRangeDateSelectionState(props: RangeDateSelectionOptions): RangeDateSelectionState {
	const inputTimeZone = useDefaultTimeZone(props.value?.start || props.defaultValue?.start || props.placeholderValue);
	...
	const minValue = props.minValue?.isAfter(viewportInterval.start) ? props.minValue : viewportInterval.start;
	const maxValue = props.maxValue?.isBefore(viewportInterval.end) ? props.maxValue : viewportInterval.end;

	const align = props.align ?? SECOND;
	const minDuration = Math.max(props.minDuration ?? SECOND, align);
	const maxDuration = Math.max(Math.min(props.maxDuration ?? 15 * YEAR, maxValue.diff(minValue)), minDuration);
	...
	const moveStart = (delta: number, { visualMinDuration = minDuration }: { visualMinDuration?: number } = {}) => {
		if (!draggingState.current) {
			return;
		}
		const { start, end } = draggingState.current;
	...
	const moveEnd = (delta: number, { visualMinDuration = minDuration }: { visualMinDuration?: number } = {}) => {
		if (!draggingState.current) {
			return;
		}
		const { start, end } = draggingState.current;
	...
	const zoomInterval = (
		scale: number,
		{ visualMinDuration = minDuration, fixedPoint = 0.5 }: { visualMinDuration?: number; fixedPoint?: number } = {},
	) => {
		if (!draggingState.current) {
			return;
		}

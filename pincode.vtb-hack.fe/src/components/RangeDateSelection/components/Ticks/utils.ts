...
import { getLerpCoeff } from "../../utils/span.ts";
...
function makeTickSizes(periodSize: number, periodName: DurationUnit, ...multipliers: number[]): TickSize[] {
	...
	while (cursor.valueOf() < viewportEnd.valueOf()) {
		...
		if (
			...
			while (next.valueOf() <= maxTickValue) {
				...
				next = next.add(tickSize.multiplier * k++, tickSize.period).add(timeDiff, "minutes");
			}
		}
...
export function makeUnavailableTicksGeometry({ tickHeight, viewportHeight }: MakeTicksOptions): Geometry {
	const w = tickHeight;
	const h = viewportHeight;
	return (x) => `M${x},0l${h},${h}l${w},0l${-h},${-h}l${-w},0`;
}

export function calculatePosition(value: DateTime | undefined, interval: RangeValue<DateTime>, width: number) {
	if (!value) {
		return NaN;
	}
